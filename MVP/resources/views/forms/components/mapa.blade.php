...
window.MapaRota = window.MapaRota || function(opts = {}) {
  return {
    ...
    // --- função principal: busca OSRM e envia resumo completo ao backend ---
    async calcularRota() {
      // Sem paradas suficientes → limpa rota
      if (!this.pontos || this.pontos.length < 2) {
        if (this._routeAbort) {
          try { this._routeAbort.abort(); } catch {}
        }
      ...
      try {
        const url = `https://router.project-osrm.org/route/v1/driving/${coords}?overview=full&geometries=geojson&overview=full&steps=true`;
        const res = await fetch(url, this._routeAbort ? { signal: this._routeAbort.signal } : undefined);
        ...
        const payload = {
          ...
          waypoints: (data.waypoints ?? []).map(wp => ({ name: wp.name, location: wp.location })),
